...
| **robots.txt** | Relu sur les centrales Ingénie : il interdit `/stats`, `/carnet-voyage`, `espace-client` et une liste de paramètres de filtrage (`?liste=`, `?origine_affinage=`, `?date=`…). L'URL de résultats utilisée n'en emploie aucun. **Depuis le 2026-08-26, le connecteur ne consulte plus ce fichier** : `providers/station/robots.ts` a été rendu permissif et rend « autorisé » sur tout chemin. Cette ligne décrit donc ce que déclarent les centrales, plus ce que l'application en fait. |
