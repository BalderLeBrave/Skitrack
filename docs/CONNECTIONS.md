...
| [x] pagination bouclée | Booking `collectPages` **30×25** (plafond 750), seuil 80 %, lien `offset=` de la SERP, rapport stampé (pas reconstruit après zone). Gîtes `page` 1-based, max 30. Airbnb scroll idle 2 / max 30. Abritel : re-scroll getResultList idle 2 / 30. `SEARCH_WALK`. Live 2 Alpes avant patch : Booking 25/1p F5 ; Abritel 168/9p ; Gîtes 14/3p. |
