#!/usr/bin/env python3
"""HTTP worker Abritel. SOURCE=vrbo-web. Isolé de Booking / Gîtes."""
...
SOURCE = os.environ.get("SOURCE", "vrbo-web")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args: object) -> None:
        sys.stderr.write("abritel-worker " + (fmt % args) + "\n")
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path.split("?", 1)[0] == "/health":
            self._send(200, {"ok": True, "source": SOURCE, "isolated": True, "engine": "cozy-door"})
            return
...
def main() -> None:
    ...
    sys.stderr.write(f"abritel-worker {SOURCE} :{PORT}\n")
    httpd.serve_forever()
