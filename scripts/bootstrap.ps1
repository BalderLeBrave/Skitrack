...
Write-Host 'Installation de pyairbnb (scraper Airbnb isole)...'
$airbnbReq = Join-Path $root 'scrape\airbnb\requirements.txt'
if (Test-Path $airbnbReq) {
    & $venvPython -m pip install -r $airbnbReq --quiet --prefer-binary
    if ($LASTEXITCODE -ne 0) { throw 'Echec de l installation pyairbnb' }
}
