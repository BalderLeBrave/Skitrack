 ...
 *
 *   node scripts/import-france-montagnes.mjs
 *   npm run catalogue:import
 *
 * Lit `docs/sources/stations-ski-france-montagnes.xlsx` — la copie versionnée du
 * fichier de travail — et écrit `src/lib/franceMontagnes.data.ts`.
 ...
 *
 * La lecture du `.xlsx` elle-même vit dans `scripts/xlsx.mjs` : elle est
...
const TARGET = 'src/lib/franceMontagnes.data.ts'
...
// Le classeur a porté des lignes en double — « Chamonix Mont-Blanc » et
// « Chamonix-Mont-Blanc », mêmes coordonnées. On ne les retire pas ici : le
// fichier généré reste le miroir du classeur, et la correction se fait dans le
// classeur. On les signale, pour qu'elles ne passent pas inaperçues.
const seenName = new Map()
for (const s of stations) {
  const key = s.fmName.normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-')
  const first = seenName.get(key)
  if (first) console.warn(`  doublon : « ${first} » et « ${s.fmName} » se réduisent au même identifiant`)
  else seenName.set(key, s.fmName)
}
 ...
 * son village et le domaine auquel elle appartient. L'appariement avec le
 * référentiel Skiinfo du dépôt vit dans \`src/lib/classeur.ts\` ; la bascule et
 * ses garanties d'identifiants dans \`src/lib/stationMigration.ts\`. Voir
 * \`docs/REFERENTIEL.md\`.
 *
