...
import { resolveWorkerBase, scrapeAirbnbOnWorker, scrapeDockerRequired } from './providers/scrapeWorker/client'
...
function registerIpc(): void {
  ...
  // Airbnb : pyairbnb (process Python isolé), puis Omkar, puis Playwright.
  ipcMain.handle(
    IPC.airbnbScrape,
    async (_e, params: AirbnbScrapeParams): Promise<AirbnbScrapeOutcome> => {
      ...
      const worker = await resolveWorkerBase('airbnb')
      const outcome = worker
        ? await scrapeAirbnbOnWorker(worker, airbnbParams).catch((err) => {
            if (scrapeDockerRequired()) throw err
            return scrapeAirbnbSearch(airbnbParams)
          })
        : await scrapeAirbnbSearch(airbnbParams)
