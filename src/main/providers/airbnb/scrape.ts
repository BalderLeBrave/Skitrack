...
import { scrapeAirbnbViaPyairbnb } from './pyairbnb'
...
export interface AirbnbScrapeResult {
  ...
  attempts?: number
  via?: 'pyairbnb' | 'pyairbnb+stl' | 'omkar' | 'playwright'
  pagesFetched?: number
...
function profileDir(): string {
  ...
  if (root) return join(root, 'airbnb')
...
export async function scrapeAirbnbSearch(
  ...
  const viaPy = await scrapeAirbnbViaPyairbnb(params)
  if (viaPy?.ok) return viaPy
