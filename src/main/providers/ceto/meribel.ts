 ...
 *
 * SERP + grille d'occupation de la fiche. Sans grille, le parseur SERP
 * décide (`partir` → partial, `/ séjour` → total_confirmed).
 */
...
import { cetoStayConfidence, priceForGroupIn, type FicheOccupancy } from './occupancy'
...
function toAccommodation(
  ...
  const confidence = cetoStayConfidence({
    grid,
    groupPrice,
    serpConfidence: item.priceConfidence,
    total
  })
  return {
