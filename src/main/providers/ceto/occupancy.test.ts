...
console.log('\n7. Confiance Ceto — SERP /séjour confirmé, « partir » partial')
check(
  'SERP /séjour sans grille → total_confirmed',
  cetoStayConfidence({ serpConfidence: 'total_confirmed', total: 2215 }) === 'total_confirmed'
)
check(
  'SERP « à partir de » sans grille → partial (Plagne ne force plus tout en partial)',
  cetoStayConfidence({ serpConfidence: 'partial', total: 1161 }) === 'partial'
)
check(
  'montant SERP sans drapeau partir → confirmé',
  cetoStayConfidence({ total: 2430 }) === 'total_confirmed'
)
check(
  'grille sans tarif du groupe → unknown, pas le teaser',
  cetoStayConfidence({ grid, groupPrice: null, serpConfidence: 'partial', total: 1161 }) === 'unknown'
)
check(
  'grille avec tarif du groupe → confirmé',
  cetoStayConfidence({ grid, groupPrice: 2736, serpConfidence: 'partial', total: 1161 }) ===
    'total_confirmed'
)
