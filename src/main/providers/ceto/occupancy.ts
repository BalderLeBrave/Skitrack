...
/**
 * Confiance du tarif Orchestra, commune aux quatre connecteurs Ceto.
 *
 * La grille de fiche prime : tarif du groupe demandé → confirmé, sinon muet
 * (jamais le « à partir de » d'une autre occupation). Sans grille, on croit
 * le parseur SERP : « partir » = `partial`, « / séjour » = `total_confirmed`.
 * Plagne / Méribel / Megève forçaient `partial` dès qu'un montant SERP
 * existait, ce qui enterrait les totaux de séjour déjà datés.
 */
export function cetoStayConfidence(args: {
  grid?: FicheOccupancy | null
  groupPrice?: number | null
  serpConfidence?: 'total_confirmed' | 'partial' | null
  total?: number
}): 'total_confirmed' | 'partial' | 'unknown' {
  if (args.grid != null) {
    return args.groupPrice != null ? 'total_confirmed' : 'unknown'
  }
  if (args.serpConfidence === 'partial') return 'partial'
  if (args.total != null && Number.isFinite(args.total) && args.total > 0) return 'total_confirmed'
  return 'unknown'
}
