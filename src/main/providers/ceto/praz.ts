/**
 * Connecteur Orchestra — Praz-sur-Arly (booking.prazsurarly.com).
 *
 * Même SERP que Megève. Pas de grille « chambre » sur les meublés :
 * le tarif SERP reste un « à partir de » (`partial`), mais il est daté.
 */
...
import { isPrazCentral } from './hosts'
import { extractChamonixMulti, type ChamonixListing } from './chamonixExtract'

export const CETO_PRAZ_PROVIDER_NAME = 'ceto-praz'
...
function toAccommodation(item: ChamonixListing, params: SearchParams): Accommodation {
  ...
  return {
    source: CETO_PRAZ_PROVIDER_NAME,
    ...
    priceConfidence: item.priceConfidence === 'partial' || total != null ? 'partial' : 'unknown',
...
export function createCetoPrazProvider(): AccommodationProvider {
  const name = CETO_PRAZ_PROVIDER_NAME
  return {
    name,
    async search(params: SearchParams): Promise<Accommodation[]> {
      const central = params.officialUrl?.trim()
      if (!central || !isPrazCentral(central)) return []
    ...
    async health(): Promise<ProviderHealth> {
      return {
        ...
        detail: breaker.open
          ? breaker.reason
          : 'Centrale Praz-sur-Arly (Orchestra) — SERP datée'
      }
    }
  }
}
