...
/** Offre la moins chère dont le **total de séjour** est confirmé, `null` sinon. */
export function cheapestOffer(property: Property): Offer | null {
  const priced = property.offers.filter((o) => comparableStayTotal(o) != null)
  if (priced.length === 0) return null
  return priced.reduce((best, offer) => {
    const value = comparableStayTotal(offer) ?? Infinity
    const current = comparableStayTotal(best) ?? Infinity
