 ...
 *     booking-web        Omkar HTTP (15 pages) puis Bright Data / Playwright
...
import { compareByStayTotal } from '@shared/stayPrice'
...
import { withOptionalWorker } from './scrapeWorker/wrap'
...
export function buildEngine(options: EngineOptions): SearchEngine {
  ...
  if (options.enableWebScrape) {
    next.register(withOptionalWorker(createBookingWebProvider({ vault: options.vault })))
    next.register(withOptionalWorker(createGitesWebProvider()))
    next.register(withOptionalWorker(createVrboWebProvider()))
    next.register(withOptionalWorker(createStationProvider()))
...
export async function aggregateResults(
  ...
  /** Prix comparable d'une offre, `null` si ce n'est pas un total de séjour confirmé. */
  const listings = outcomes.flatMap((outcome) => outcome.results).sort(compareByStayTotal)
