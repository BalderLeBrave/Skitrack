...
import { scrapeBookingViaStealth } from '../booking/stealth'
import {
  abritelCanonicalUrl,
...
function waitSelectorOf(name: string): string {
  if (name === 'gites-web') return GITES_CARD_SELECTORS.join(', ')
  if (name === 'vrbo-web' || name === 'expedia-web') return VRBO_CARD_SELECTORS.join(', ')
  return BOOKING_CARD_SELECTORS.join(', ')
}

function scrollSelectorsOf(name: string): string[] {
  if (name === 'gites-web') return GITES_CARD_SELECTORS
  if (name === 'vrbo-web' || name === 'expedia-web') return VRBO_CARD_SELECTORS
  return BOOKING_CARD_SELECTORS
}
...
async function loadAndExtract(
  ...
  try {
    await page.waitForSelector(waitSelectorOf(name), { timeout: 8_000 })
  } catch {
  ...
  if (cards.length < 25) {
    await scrollToEnd(page, 8, scrollSels)
  ...
  if (cards.length === 0 && (await looksBlocked(page))) {
    ...
    if (solved) {
      await sleep(800)
      await scrollToEnd(page, 6, scrollSels)
      cards = await page.evaluate(extract)
    }
    if (cards.length === 0 && (await looksBlocked(page))) {
      throw new Error('relevé refusé par la source (captcha ou blocage anti-robot)')
            ...
            const collected =
              pageStep > 0
                ? await collectPages(
                    ...
                    pageStep,
                    (url) => loadAndExtract(page, url, timeoutMs, extract, name),
                    maxPages
                  )
                : await loadAndExtract(page, buildUrl(params), timeoutMs, extract, name)
...
export function createBookingWebProvider(opts?: ScrapeAttemptOptions & {
    ...
    const cards = await withPage(
      ...
      async (page) => {
        const collected = await collectBookingPages(
          ...
          (url) => loadAndExtract(page, url, timeoutMs, extractBookingCards, name),
  ...
  return {
    ...
    async search(params: SearchParams): Promise<Accommodation[]> {
      ...
      const viaStealth = await scrapeBookingViaStealth(params, opts?.vault)
      if (viaStealth && viaStealth.length > 0) return viaStealth
      ...
      return {
        ...
        detail: omkar
          ...
          : bd
            ...
            : `stealth Python (invisible_playwright → Camoufox → UC) puis Playwright, ${BOOKING_MAX_PAGES} page(s) au plus`
      }
    }
  }
}
