...
/** Un Chromium par scraper : un retry Booking ne ferme plus Gîtes. */
const scraperSlots = new Map<string, ScraperSlot>()
const scraperLaunchLocks = new Map<string, Promise<BrowserContext>>()

function profileDir(scraper: string): string {
  const safe = scraper.replace(/[^a-z0-9._-]+/gi, '_') || 'default'
  const root = process.env.SKITRACK_SCRAPE_PROFILE
  if (root) return join(root, safe)
  return join(app.getPath('userData'), 'webscrape-browser-profile', safe)
}
...
function contextAlive(ctx: BrowserContext | null | undefined): ctx is BrowserContext {
  if (!ctx) return false
  try {
    void ctx.pages()
    return true
...
/**
 * Recycle-t-on le Chromium **de ce scraper** après un essai raté ?
 *
 * Un retry Booking ne doit fermer que Booking. Les onglets Gîtes / Abritel /
 * centrale vivent dans d'autres contextes et ne sont pas comptés ici.
 */
export function shouldRecycleScraperContext(opts: {
  ownPages: number
  rotateProxy: boolean
}): boolean {
  if (!opts.rotateProxy) return false
  return opts.ownPages <= 1
}

/** Ancien nom, conservé pour les tests. Compte les onglets **du** scraper. */
export function shouldCloseSharedContext(opts: {
  openPages: number
  rotateProxy: boolean
}): boolean {
  return shouldRecycleScraperContext({ ownPages: opts.openPages, rotateProxy: opts.rotateProxy })
}
...
async function launchContext(
  ...
  if (process.env.SKITRACK_SCRAPE_PROFILE) {
    ctx = await chromium.launchPersistentContext(profileDir(scraper), {
      ...common,
      args: [...(common.args as string[]), '--no-sandbox', '--disable-gpu']
    })
  } else {
    try {
      ctx = await chromium.launchPersistentContext(profileDir(scraper), {
...
export async function getScrapeContext(
  ...
  const slot = scraperSlots.get(scraper)

  if (slot && contextAlive(slot.ctx)) {
    if (slot.proxyRaw === desiredRaw) return slot.ctx
    if (slot.ctx.pages().length > 0) return slot.ctx
  }

  const existing = scraperLaunchLocks.get(scraper)
  if (existing) return existing

  const pending = (async () => {
    const current = scraperSlots.get(scraper)
    if (current && contextAlive(current.ctx) && current.proxyRaw === desiredRaw) {
      return current.ctx
    }
    if (current) {
      try {
      ...
      scraperSlots.delete(scraper)
    }
    const ctx = await launchContext(headless, desiredProxy, scraper)
    scraperSlots.set(scraper, { ctx, proxyRaw: desiredRaw })
    return ctx
  })()
  scraperLaunchLocks.set(scraper, pending)
  try {
    return await pending
  } finally {
    if (scraperLaunchLocks.get(scraper) === pending) scraperLaunchLocks.delete(scraper)
  }
}

export async function closeWebscrapeBrowser(scraper?: string): Promise<void> {
  const ids = scraper ? [scraper] : [...scraperSlots.keys()]
  await Promise.all(
    ids.map(async (id) => {
      const slot = scraperSlots.get(id)
      try {
        await slot?.ctx.close()
      } catch {
        /* ignore */
      }
      scraperSlots.delete(id)
      scraperLaunchLocks.delete(id)
    })
  )
}
...
export async function withPage<T>(
  ...
  const ctx = await getScrapeContext(headless, proxy === null ? null : proxy, scraper)
...
/**
 * N onglets d'un **même** scraper, réutilisés. Ceto ouvrait un onglet par fiche
 * (`withPage` × 40) : launch est partagé au scraper, pas entre sources.
 */
export async function withPagePool<T>(
  ...
  const ctx = await getScrapeContext(headless, undefined, scraper)
  const pages: Page[] = []
  try {
...
export async function withRetries<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const slot = scraperSlots.get(label)
      const ownPages = slot && contextAlive(slot.ctx) ? slot.ctx.pages().length : 0
      if (shouldRecycleScraperContext({ ownPages, rotateProxy: true })) {
        try {
          await closeWebscrapeBrowser(label)
        } catch {
          // ignore
        }
      }
...
export function webscrapePriceFields(
  ...
  // Dispatch strict : le libellé Gîtes ne requalifie pas Booking, et inversement.
  if (source === 'gites-web' || source === 'gites-de-france') {
    return { weeklyPrice: price }
  }
  if (source === 'cozycozy-web' || source === 'cozycozy') {
    return { nightlyPrice: price }
  }
  if (source === 'booking-web' || source === 'booking') {
    return looksNightlyPriceText(priceText) ? { nightlyPrice: price } : { totalPrice: price }
  }
  if (looksWeeklyFromPriceText(priceText)) return { weeklyPrice: price }
  if (looksNightlyPriceText(priceText)) return { nightlyPrice: price }
 ...
 */
export const BOOKING_CARD_SELECTORS = [
  '[data-testid="card-container"]',
  '[data-testid="property-card"]'
]
export const GITES_CARD_SELECTORS = ['.js-search-tile', '.g2f-accommodationTile']
export const VRBO_CARD_SELECTORS = ['[data-stid="property-listing"]', '.uitk-card', '[itemprop="itemListElement"]']
...
export async function scrollToEnd(
  ...
  const sels = cardSelectors?.length
    ? cardSelectors
    : [...BOOKING_CARD_SELECTORS, ...GITES_CARD_SELECTORS, ...VRBO_CARD_SELECTORS]
  let previous = -1
  let idle = 0
  for (let step = 0; step < maxSteps; step++) {
    const height = await page.evaluate((sels: string[]) => {
