...
export function bookingSearchUrl(params: SearchParams, offset = 0): string {
  ...
  // Total de séjour affiché, pas un tarif à la nuit.
  u.searchParams.set('sb_price_type', 'total')
  u.searchParams.set('lang', 'fr')
  // src=index : comme un clic depuis l’accueil, pas un deep-link froid.
  u.searchParams.set('src', offset > 0 ? 'searchresults' : 'index')
