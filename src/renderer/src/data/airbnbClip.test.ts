...
check('à partir de /nuit n’est pas un séjour', parseAirbnbPrice('À partir de 89 €/nuit') === null)
check('89 €/nuit sans au total → null', parseAirbnbPrice('89 €/nuit') === null)
check('libellé nu sans au total → null', parseAirbnbPrice('200 €') === null)
check(
  'espace insécable + au total (dump 2 Alpes)',
  parseAirbnbPrice('3\u202f469\u00a0€ au total') === 3469
)
...
check('tableau nu accepté', parseAirbnbClipboard('[{"id":"9","name":"X","priceLabel":"200 € au total"}]').listings.length === 1)
