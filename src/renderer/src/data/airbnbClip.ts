 ...
 *
 * Un « à partir de » ou un tarif /nuit sans « au total » n'est pas un séjour.
 * Le marque-page écrit toujours « N € au total » ; un libellé nu n'est pas
 * comparable.
 */
export function parseAirbnbPrice(label: string | undefined): number | null {
  if (!label) return null
  if (/(?:à|a)\s+partir\s+de|\bfrom\s+\d/i.test(label)) return null

  // Le séjour, pas la nuit : le marque-page écrit « N € au total ».
  const stay =
