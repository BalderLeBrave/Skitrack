...
const CATALOG = {
  ...
  sb_go: ['Rechercher', 'Search'],
  sb_rooms: ['Chambres', 'Bedrooms'],
  sb_rooms_any: ['Toutes', 'Any'],
  sb_rooms_less: ['Une chambre de moins', 'One bedroom fewer'],
  sb_rooms_more: ['Une chambre de plus', 'One bedroom more'],
  // --- Recomposition (shell, écrans E1–E5) ------------------------------------
  rc_nav_more: ['Plus', 'More'],
  rc_nav_journey: ['Parcours', 'Journey'],
  rc_nav_compare: ['Comparer', 'Compare'],
  rc_nav_reservation: ['Réservation', 'Booking'],
  rc_nav_lodgings_locked: ['Choisissez d’abord une station.', 'Pick a resort first.'],
  rc_sb_station: ['Station', 'Resort'],
  rc_sb_dates: ['Dates', 'Dates'],
  rc_sb_dates_any: ['Choisir les dates', 'Pick dates'],
  rc_price_unpriced: ['Prix non extrait pour ces dates', 'Price not extracted for these dates'],
  rc_price_other_dates: ['Tarifé pour d’autres dates', 'Priced for other dates'],
  rc_price_gone: ['Annonce disparue à ces dates', 'Listing gone for these dates'],
  rc_price_unrated: ['Prix à vérifier chez la source', 'Check price at the source'],
  rc_price_stay: ['séjour · {n} nuits', 'stay · {n} nights'],
  rc_price_pp: ['/ pers.', '/ person'],
  rc_price_firm: ['Prix ferme pour vos dates', 'Firm price for your dates'],
  rc_station_open: ['Ouvrir la fiche de {d}', 'Open the {d} sheet'],
  rc_fact_km: ['Pistes', 'Runs'],
  rc_fact_alt: ['Altitude', 'Altitude'],
  rc_fact_village: ['Front de neige', 'Snow front'],
  rc_fact_pass: ['Forfait 6 j', '6-day pass'],
  rc_saved: ['Retenue', 'Saved'],
  rc_save: ['Retenir', 'Save'],
  rc_cmp_empty_title: ['Aucune station cochée', 'No resort ticked'],
  rc_cmp_empty_hint: ['Cochez « Comparer » sur deux stations ou plus : elles s’alignent ici, critère par critère.', 'Tick “Compare” on two or more resorts: they line up here, criterion by criterion.'],
  rc_cmp_snow: ['Enneigement (bas / haut)', 'Snow depth (base / top)'],
  rc_cmp_lifts: ['Remontées', 'Lifts'],
  rc_cmp_median: ['Prix médian logements (relevé)', 'Median lodging price (collected)'],
  rc_cmp_travel: ['Trajet voiture', 'Drive'],
  rc_cmp_linked: ['Forfait relié', 'Linked pass'],
  rc_cmp_glacier: ['Glacier', 'Glacier'],
  rc_cmp_region: ['Massif · département', 'Range · area'],
  rc_yes: ['oui', 'yes'],
  rc_no: ['non', 'no'],
  rc_cmp_criteria: ['Critère', 'Criterion'],
  rc_cmp_sheet: ['Fiche', 'Sheet'],
  rc_cmp_remove: ['Retirer {d} de la comparaison', 'Remove {d} from comparison'],
  rc_cmp_note: ['« — » : donnée absente du référentiel ou non relevée. Rien n’est estimé sans le dire.', '“—”: missing from the reference set or not collected. Nothing is estimated silently.'],
  rc_cmp_cta: ['Voir les logements disponibles à {d}', 'See available stays in {d}'],
  ...
  rc_lodge_open: ['Ouvrir sur {s}', 'Open on {s}'],
  rc_ck_title: ['Demande de réservation', 'Booking request'],
  rc_ck_lead: ['Vérifiez le récapitulatif, puis confirmez : la demande s’ouvre chez la source qui a publié l’annonce.', 'Check the summary, then confirm: the request opens at the source that published the listing.'],
  rc_ck_recap: ['Récapitulatif', 'Summary'],
  rc_ck_station: ['Station', 'Resort'],
  rc_ck_lodging: ['Logement', 'Stay'],
  rc_ck_source: ['Source', 'Source'],
  rc_ck_lodging_cost: ['Hébergement', 'Lodging'],
  rc_ck_passes: ['Forfaits', 'Passes'],
  rc_ck_pers: ['pers.', 'people'],
  rc_ck_route: ['Route', 'Drive'],
  rc_ck_total: ['Total séjour', 'Stay total'],
  rc_ck_not_firm: ['Le prix de ce logement n’est pas ferme pour vos dates : le total reste ouvert.', 'This stay’s price is not firm for your dates: the total stays open.'],
  rc_ck_deeplink: ['Voir l’annonce sur {s}', 'See the listing on {s}'],
  rc_ck_form: ['Vos coordonnées', 'Your details'],
  rc_ck_form_note: ['SKITRACK n’envoie rien : « Confirmer » copie ce récapitulatif et ouvre l’annonce sur {s}, où la réservation se fait.', 'SKITRACK sends nothing: “Confirm” copies this summary and opens the listing on {s}, where the booking happens.'],
  rc_ck_name: ['Nom', 'Name'],
  rc_ck_email: ['E-mail', 'E-mail'],
  rc_ck_note: ['Message', 'Message'],
  rc_ck_confirm: ['Confirmer la demande', 'Confirm the request'],
  rc_ck_copied: ['Récapitulatif copié. L’annonce s’ouvre chez la source.', 'Summary copied. The listing opens at the source.'],
  rc_stat_median_note: ['sur {n} forfaits relevés', 'over {n} recorded passes'],
  rc_home_empty_title: ['Référentiel vide', 'Empty reference set'],
  rc_home_empty_hint: ['Aucune station chargée : importez un référentiel depuis Réglages.', 'No resort loaded: import a reference file from Settings.'],
  rc_chip_linked: ['Forfait relié', 'Linked pass'],
  rc_step_1: ['Étape 1 · Station', 'Step 1 · Resort'],
  rc_step_2: ['Étape 2 · Logement', 'Step 2 · Stay'],
  rc_step_3: ['Étape 3 · Réservation', 'Step 3 · Booking'],
  rc_cmp_title: ['Comparer les stations', 'Compare resorts'],
  rc_cmp_lead: ['Données du référentiel et relevés du jour. Cochez, comparez, puis ouvrez les logements de la station choisie — dates et groupe suivent.', 'Reference data and same-day readings. Tick, compare, then open stays for the chosen resort — dates and group follow.'],
  rc_cmp_table: ['Tableau comparatif', 'Comparison table'],
  rc_filters: ['Filtres', 'Filters'],
  rc_cmp_count: ['{n} stations sur {t}', '{n} of {t} resorts'],
  rc_reset: ['Tout effacer', 'Clear all'],
  rc_map: ['Carte', 'Map'],
  rc_cmp_none_title: ['Aucune station ne passe ces filtres', 'No resort matches these filters'],
  rc_cmp_none_hint: ['Relâchez un critère : la liste revient.', 'Loosen one criterion and the list comes back.'],
  rc_cmp_more: ['+ {n} stations non affichées : affinez les filtres.', '+ {n} resorts not shown: refine the filters.'],
  rc_crumbs: ['Fil d’Ariane', 'Breadcrumb'],
  rc_snow_none: ['Aucun relevé de neige pour ce domaine.', 'No snow reading for this resort.'],
  rc_sticky_title: ['Votre séjour', 'Your stay'],
  rc_station_cta: ['Voir les logements', 'See stays'],
  rc_sticky_note: ['Dates et groupe se modifient dans la barre du haut et suivent jusqu’à la réservation.', 'Dates and group are edited in the top bar and follow through to booking.'],
  rc_sort_pp: ['Prix / pers.', 'Price / person'],
  rc_sort_total: ['Total séjour', 'Stay total'],
  rc_sort_dist: ['Distance aux pistes', 'Distance to runs'],
  rc_sort_note: ['Note', 'Rating'],
  rc_lodg_title: ['Logements à {d}', 'Stays in {d}'],
  rc_lodg_group: ['{p} voyageurs · chambres : {r}', '{p} travellers · bedrooms: {r}'],
  rc_lodg_searching: ['Recherche… {s} s', 'Searching… {s} s'],
  rc_lodg_refresh: ['Relancer le relevé', 'Run the search again'],
  rc_lodg_rule_avail: ['Disponible', 'Available'],
  rc_lodg_rule_cap: ['≥ {n} pers.', '≥ {n} guests'],
  rc_lodg_rule_rooms: ['≥ {n} chambres', '≥ {n} bedrooms'],
  rc_lodg_rule_firm: ['Prix ferme', 'Firm price'],
  rc_lodg_rule_note: ['Seules les annonces qui remplissent ces quatre conditions pour vos dates sont listées.', 'Only listings meeting all four conditions for your dates are shown.'],
  rc_lodg_need_dates: ['Choisissez vos dates et votre groupe dans la barre du haut pour lancer le relevé.', 'Pick your dates and group in the top bar to start the search.'],
  rc_lodg_count: ['{n} logements', '{n} stays'],
  rc_lodg_count_one: ['{n} logement', '{n} stay'],
  rc_lodg_group_one: ['{p} voyageur · chambres : {r}', '{p} traveller · bedrooms: {r}'],
  rc_lodg_budget: ['≤ {n} €', '≤ €{n}'],
  rc_lodg_empty_title: ['Aucune annonce relevée pour ces dates', 'No listing collected for these dates'],
  rc_lodg_empty_hint: ['Les sources n’ont rien retourné ou n’ont pas encore été interrogées.', 'Sources returned nothing or have not been queried yet.'],
  rc_lodg_counters: ['Compteurs du relevé', 'Collection counters'],
  rc_lodg_best_price: ['Le moins cher / pers.', 'Cheapest per person'],
  rc_lodg_closest: ['Le plus près des pistes', 'Closest to the slopes'],
  rc_lodg_ribbon_snow: ['Neige bas / haut', 'Snow base / top'],
  rc_lodg_ribbon_sheet: ['Fiche station →', 'Resort sheet →'],
  rc_lodge_nophoto_src: ['Photo non fournie par {s}', 'No photo provided by {s}'],
  rc_stay_title: ['Votre séjour', 'Your stay'],
  rc_stay_kept: ['Logement retenu', 'Chosen stay'],
  rc_stay_cheapest: ['Le moins cher affiché', 'Cheapest shown'],
  rc_stay_none: ['Aucun logement à prix ferme pour ces dates', 'No firm-priced stay for these dates'],
  rc_stay_go: ['Demander la réservation', 'Request the booking'],
  rc_stay_pp: ['par personne', 'per person'],
  rc_stay_edit: ['Modifier', 'Edit'],
  rc_wx_today: ['Météo du jour', 'Today’s weather'],
  rc_wx_top: ['sommet', 'top'],
  rc_wx_base: ['bas des pistes', 'base'],
  rc_wx_snow24: ['neige 24 h', 'snow 24 h'],
  rc_cnt_collected: ['annonces collectées', 'listings collected'],
  rc_cnt_shown: ['affichées (disponibles, capacité, chambres, prix ferme)', 'shown (available, capacity, bedrooms, firm price)'],
  rc_cnt_unpriced: ['écartées : prix non extrait (hidden_extraction_failed) — l’annonce existe, son prix n’a pas pu être lu', 'set aside: price not extracted (hidden_extraction_failed) — the listing exists, its price could not be read'],
  rc_cnt_other_dates: ['écartées : tarifées pour d’autres dates', 'set aside: priced for other dates'],
  rc_cnt_gone: ['écartées : disparues à ces dates', 'set aside: gone for these dates'],
  rc_cnt_filtered: ['masquées par vos filtres (budget, type, source)', 'hidden by your filters (budget, type, source)'],
  rc_cnt_failed: ['Sources en échec : {s}', 'Failed sources: {s}'],
  rc_cnt_empty: ['Sources sans résultat : {s}', 'Sources with no result: {s}'],
  rc_res_missing_title: ['Logement introuvable', 'Stay not found'],
  rc_res_missing_hint: ['Il a quitté le relevé ou l’adresse est incomplète.', 'It left the collection or the address is incomplete.'],
  home_popular: ['Stations populaires', 'Popular resorts'],
  home_popular_note: [
    'Les plus grands domaines du référentiel, classés par kilomètres de pistes. L’enneigement n’apparaît que si un relevé du jour existe.',
    'The largest resorts in the reference set, ranked by kilometres of runs. Snow depth only shows when a same-day reading exists.'
  ],
  home_popular_compare: ['Comparer', 'Compare'],
  home_popular_comparing: ['À comparer', 'Comparing'],
  home_popular_compare_go: ['Comparer {n} stations →', 'Compare {n} resorts →'],
  home_popular_compare_one: ['Cochez une 2ᵉ station pour comparer', 'Tick a 2nd resort to compare'],
  home_popular_open: ['Ouvrir la fiche de {d}', 'Open the {d} sheet'],
  home_popular_km: ['km de pistes', 'km of runs'],
  home_popular_snow: ['neige bas / haut', 'snow base / top'],
  home_popular_alt: ['altitude', 'altitude'],
  ...
  basemap_pistes: ['Montagne', 'Mountain'],
  basemap_pistes_sub: [
    'relief, courbes de niveau, sommets — OpenTopoMap, sans clé',
    'relief, contours, peaks — OpenTopoMap, no key'
  ],
  basemap_ign: ['Plan IGN', 'IGN map'],
  basemap_ign_sub: [
    'topographie française, hameaux et accès',
    'French topography, hamlets and access'
  ],
  basemap_ortho: ['Photo aérienne', 'Aerial photo'],
  basemap_ortho_sub: [
    'orthophoto IGN — pentes, forêts, barres rocheuses',
    'IGN orthophoto — slopes, forests, rock faces'
  ],
  basemap_sat: ['Satellite', 'Satellite'],
  basemap_sat_sub: [
    'imagerie mondiale — frontières suisse et italienne',
    'worldwide imagery — Swiss and Italian borders'
  ],
  basemap_sobre: ['Sobre', 'Plain'],
  basemap_sobre_sub: [
    'fond clair — les prix et les épingles ressortent',
    'light background — prices and pins stand out'
  ],
  ...
  import_open: ['Importer une annonce', 'Import a listing'],
  import_title: ['Compléter l’annonce', 'Complete the listing'],
  import_fetch: ['Lire la page', 'Read the page'],
  import_fetching: ['Lecture…', 'Reading…'],
  import_paste_jsonld: ['Coller JSON-LD', 'Paste JSON-LD'],
  import_parse: ['Relire le JSON-LD', 'Re-parse JSON-LD'],
  import_section_required: ['Informations obligatoires', 'Required information'],
  import_section_geo: ['Localisation', 'Location'],
  import_section_fees: ['Frais et charges', 'Fees and charges'],
  import_section_raw: ['Données brutes', 'Raw data'],
  import_listing_name: ['Titre de l’annonce', 'Listing title'],
  import_price_from: ['Prix à partir de', 'Price from'],
  import_unit: ['Unité', 'Unit'],
  import_unit_night: ['€ / nuit', '€ / night'],
  import_unit_week: ['€ / semaine', '€ / week'],
  import_unit_stay: ['€ / séjour', '€ / stay'],
  import_unit_unknown: ['inconnue', 'unknown'],
  import_refundable: ['Caution remboursable', 'Refundable deposit'],
  import_fees_complete: ['Charges connues — le total est comparable', 'Fees known — the total is comparable'],
  import_no_geo: ['Pas de position — pas de badge skis aux pieds', 'No location — no ski-in badge'],
  import_geocode: ['Localiser l’adresse', 'Locate the address'],
  import_place_map: ['Cliquez sur la carte pour placer le logement', 'Click the map to place the stay'],
  import_validate: ['Enregistrer', 'Save'],
  import_cancel: ['Annuler', 'Cancel'],
  import_incomplete_price: ['Prix incomplet', 'Incomplete price'],
  import_call_price: ['Prix d’appel', 'Headline price'],
  import_source: ['source', 'source'],
  import_confidence: ['confiance', 'confidence'],
  import_status_partial: ['Lecture partielle — complétez les champs manquants.', 'Partial read — fill in the missing fields.'],
  import_status_denied: ['Page inaccessible — saisie manuelle.', 'Page unreachable — enter the listing by hand.'],
  import_score: ['Complétude {n} %', 'Completeness {n}%'],
  import_lat: ['Latitude', 'Latitude'],
  import_lon: ['Longitude', 'Longitude'],
  import_arr: ['Arrivée', 'Check-in'],
  import_dep: ['Départ', 'Check-out'],
  import_guests: ['Voyageurs', 'Guests'],
  import_cleaning: ['Ménage', 'Cleaning'],
  import_tax: ['Taxe de séjour', 'Tourist tax'],
  import_service: ['Frais de service', 'Service fee'],
  import_utilities: ['Charges', 'Utilities'],
  import_deposit: ['Caution', 'Deposit'],
