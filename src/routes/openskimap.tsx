...
import { OSM_VERDICT_FR, osmSkiinfoAll, osmSkiinfoSummary, type OsmVerdict } from "@/lib/openskimap";
...
function OpenSkiMapPage() {
  ...
  const shown = useMemo(() => {
    ...
    list.sort((a, b) => (b.nOsm ?? 0) / Math.max(1, b.nSki ?? 1) - (a.nOsm ?? 0) / Math.max(1, a.nSki ?? 1));
    return list;
  }, [rows, filter]);
    ...
    >
      <main className="mx-auto w-full max-w-5xl px-4 py-6">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted">
          OpenSkiMap = pistes OSM (même fond qu’OpenSnowMap). Ce n’est pas OpenSnow.com (prévisions US, API
          partenaire). Les comptes OSM sont des tracés ; Skiinfo publie des pistes brochure. On n’écrase pas le mix
          avec OSM.
        </p>
